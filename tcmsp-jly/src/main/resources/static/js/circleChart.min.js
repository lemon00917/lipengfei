!function(a) {
	...
	a.fn.circleChart = function(b) {
		...
		Math.linearTween = ((a, b, c, d) => c * a / d + b), Math.easeInQuad = ((a, b, c, d) => {
			...
			a--;return c / 2 * (2 - Math.pow(2, -10 * a)) + b
		}), Math.easeInCirc = ((a, b, c, d) => {
			a /= d;return -c * (Math.sqrt(1 - a * a) - 1) + b
		}), Math.easeOutCubic = ((a, b, c, d) => {
			a /= d;a--;return c * (a * a * a + 1) + b
		}), Math.easeInOutCubic = ((a, b, c, d) => {
			a /= d / 2;
			...
			a /= d;a--;return c * Math.sqrt(1 - a * a) + b
		}), Math.easeInOutCirc = ((a, b, c, d) => {
			a /= d / 2;
			if (a < 1) return -c / 2 * (Math.sqrt(1 - a * a) - 1) + b;
			a -= 2;return c / 2 * (Math.sqrt(1 - a * a) + 1) + b
		});
		...
		let e = (b, c) => {
				b.data("text", c);a(".circleChart_text", b).html(c)
			},
			f = a => a / Math.PI * 180,
			g = a => a / 180 * Math.PI,
			h = a => g(a / 100 * 360),
		...
		return this.each((e, f) => {
			...
			a("canvas.circleChart_canvas", i).length || i.append(function() {
				...
				"font-family" : m.textFamily,
				"font-size" : !1 !== m.textSize ? m.textSize : m.size * m.relativeTextSize
			})));m.autoCss && i.css("position", "relative");m.redraw || (m.cAngle = m.currentCAngle ? m.currentCAngle : m.cAngle, m.startAngle = m.currentStartAngle ? m.currentStartAngle : m.startAngle);var n = a("canvas", i).get(0);
			...
			m.animate ? 0 !== m.value ? x.animate(i, o, (new Date).getTime(), (new Date).getTime(), t > s) : j(() => {
				o.clearRect(0, 0, this.settings.size, this.settings.size);x.settings.background && x.drawBackground(o)
			}) : (x.cAngle = x.eAngle, j(() => {
