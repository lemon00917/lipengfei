...
import com.cdutcm.core.util.ReflectHelper;
			...
			if (mappedStatement.getId().matches(pageSqlId)) { // 拦截需要分页的SQL
				...
				if (parameterObject == null) {
					...
					if (parameterObject instanceof Page) { // 参数就是Page实体
						...
						page.setEntityOrField(true); // 见com.cdutcm.sys.entity.Page.entityOrField
														// 注释
						...
						if (pageField != null) {
							...
							if (page == null)
								page = new Page();
							page.setEntityOrField(false); // 见com.cdutcm.sys.entity.Page.entityOrField
															// 注释
							page.setTotalResult(count);
				...
				if (parameterMapping.getMode() != ParameterMode.OUT) {
					...
					if (parameterObject == null) {
						...
						value = boundSql.getAdditionalParameter(propertyName);
							...
							&& boundSql.hasAdditionalParameter(prop.getName())) {// ForEachSqlNode.ITEM_PREFIX
		...
		if (page != null && StringUtil.notEmpty(dialect)) {
			...
			if ("mysql".equals(dialect)) {
				...
				pageSql.append("select top " + page.getShowCount()
						+ " * from (");
				pageSql.append(sql);
				pageSql.append(") A where rownumber > "
