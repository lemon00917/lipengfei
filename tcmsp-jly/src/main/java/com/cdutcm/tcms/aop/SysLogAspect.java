...
public class SysLogAspect {
    ...
    public void controllerBefore(JoinPoint joinPoint){
    	...
    	try {
    		String info = getControllerinfos(joinPoint);
    		String[] infos = info.split(":");
    		syslog.get().setId(new IdWorker().nextId());
    		
    		syslog.get().setStarttime(new Date());
    		syslog.get().setIp(ip);
    		syslog.get().setCreateor(user.getUsername());
			syslog.get().setDescription(infos[1]);
			String params = "";  
